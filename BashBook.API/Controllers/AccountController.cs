...
using BashBook.API.Models;
using BashBook.API.Providers;
using BashBook.API.Results;
...
namespace BashBook.API.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            if (result.Succeeded)
            {
                return Ok("Email confirmarion success");
            }
            return BadRequest("Email confirmation failed");
        }
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            var response = _user.Register(new UserRegisterModel()
            {
                Mobile = model.Mobile,
                Email = model.Email
            });
            ...
            if (!result.Succeeded)
            {
                _user.Delete(response.UserId);
                return GetErrorResult(result);
            }
            //string body;
            //string code = await UserManager.GenerateEmailConfirmationTokenAsync(user.Id);
            //var callbackUrl = string.Format("http://localhost:4200/confirm-email?userId={0}&code={1}", user.Id, code);

            //using (StreamReader reader =
            //    new StreamReader(HostingEnvironment.MapPath("~/Content/Templates/Email/ConfirmEmail.html")))
            //{
            //    body = reader.ReadToEnd();
            //}
            //body = body.Replace("{{name}}", "User");
            //body = body.Replace("{{url}}", callbackUrl.ToString());
